...
public class Control {
	...
	private void printNote() {
		double ototal=0,t=0,ocollcet=0;
		Custom custom;
		...
		if ("y".equals(s)) {
			while(true) {
				...
				if (custom.getCusname()==null) {
					...
					if (i==1) {
						System.out.print("原价"+ototal+"\t普通会员九折："+ototal*0.9);
						ototal=ototal*0.9;
					}else if (i==2) {
						System.out.print("原价"+ototal+"\t超级会员八折："+ototal*0.8);
						ototal=ototal*0.8;
					}
		...
		this.v.println("\t MCD 连锁餐厅");
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		this.v.println("下单时间："+sdf.format(order.getDate()));
		this.v.println("订  单  号："+oid);
		this.v.println("-----------------------------------------");
		this.v.println("菜品名称\t购买数量\t菜品种类\t单价\t小计");
		List<Cart> list = this.ts.findCarts(oid);	
		for (Cart cart : carts) {
			Food food = this.ts.findFoodByid(cart.getFid());
			System.out.println(food.getFname()+"\t"+cart.getNum()+"\t"+food.getTypename()+"\t"+food.getFprice()+"\t"+food.getFprice()*cart.getNum());
		}
		this.v.println("-----------------------------------------");
	...
	private void outOrder() {
		String s = this.ts.outXML(emp.getEid());
		System.out.println(s);
	}
	//经理登录后的菜单
	...
	private void addMgr() {
		...
		if (mgr!=null) {
			...
			if (flag==false) {
				...
				String s=this.ts.mregist(id, account, password, name);
				this.v.println(s);
			}
		}				
	}
		...
		if (flag==false) {
			...
			if (mgr.getMname()==null) {
				...
				if (dining.getDinname()==null) {
					...
					String s=this.ts.eregist(id, account, password, name, mid, dinid);
					this.v.println(s);
				}				
			}
		}		
